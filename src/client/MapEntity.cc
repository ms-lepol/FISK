...
namespace fisk {
    ...
    void  MapEntity::configureMap1(){
        ...
        lands["Northwest Territory"] = std::make_unique<LandEntity>("Northwest Territory", pathNorthAmericaTerr, gf::Vector2i(xNorthAmericaTerr,yNorthAmericaTerr),gf::Vector2i(xTextNorthAmericaTerr,yTextNorthAmericaTerr),game_hub);
        ...
        lands["Western United States"] = std::make_unique<LandEntity>("Western United States", pathWestUS, gf::Vector2i(xWestUS,yWestUS),gf::Vector2i(xTextWestUS,yTextWestUS),game_hub);
        lands["Eastern United States"] = std::make_unique<LandEntity>("Eastern United States", pathEastUS, gf::Vector2i(xEastUS,yEastUS),gf::Vector2i(xTextEastUS,yTextEastUS),game_hub);
        lands["Central America"] = std::make_unique<LandEntity>("Central America", pathCentralAmerica, gf::Vector2i(xCentralAmerica,yCentralAmerica),gf::Vector2i(xTextCentralAmerica,yTextCentralAmerica),game_hub);
        ...
        lands["Great Britain"] = std::make_unique<LandEntity>("Great Britain", pathGB, gf::Vector2i(xGB,yGB),gf::Vector2i(xTextGB,yTextGB),game_hub);
        ...
        lands["East Africa"] = std::make_unique<LandEntity>("East Africa", pathEastAfrica, gf::Vector2i(xEastAfrica,yEastAfrica),gf::Vector2i(xTextEastAfrica,yTextEastAfrica),game_hub);
        ...
        lands["South Africa"] = std::make_unique<LandEntity>("South Africa", pathSouthAfrica, gf::Vector2i(xSouthAfrica,ySouthAfrica),gf::Vector2i(xTextSouthAfrica,yTextSouthAfrica),game_hub);
        ...
        lands["Middle East"] = std::make_unique<LandEntity>("Middle East", pathMiddleEast, gf::Vector2i(xMiddleEast,yMiddleEast),gf::Vector2i(xTextMiddleEast,yTextMiddleEast),game_hub);
        ...
        lands["New Zealand"] = std::make_unique<LandEntity>("New Zealand", pathNewZealand, gf::Vector2i(xNewZealand,yNewZealand),gf::Vector2i(xTextNewZealand,yTextNewZealand),game_hub);
        lands["Western Australia"] = std::make_unique<LandEntity>("Western Australia", pathWestAustralia, gf::Vector2i(xWestAustralia,yWestAustralia),gf::Vector2i(xTextWestAustralia,yTextWestAustralia),game_hub);
        lands["Eastern Australia"] = std::make_unique<LandEntity>("Eastern Australia", pathEastAustralia, gf::Vector2i(xEastAustralia,yEastAustralia),gf::Vector2i(xTextEastAustralia,yTextEastAustralia),game_hub);
        ...
        if (old_selection != nullptr) { //2 Choices
            ...
            TurnPhase phase  = game_hub.clientNetwork.getGameModel().get_current_phase();
                if (phase== TurnPhase::Attack){
                    auto neighbors = game_hub.clientNetwork.getGameModel().get_land_by_name(old_selection->getName()).getNeighbors();
                    for (auto& neighbor : neighbors) {
                        if (game_hub.clientNetwork.getGameModel().get_land(neighbor).getOwner() != game_hub.clientNetwork.getGameModel().get_land_by_name(old_selection->getName()).getOwner()) {
                            keysHinted.push_back(game_hub.clientNetwork.getGameModel().get_land(neighbor).getName());
                        }
                    }
                } else if (phase == TurnPhase::Reinforce) {
                    keysHinted.push_back(curr_selection->getName());
                }
